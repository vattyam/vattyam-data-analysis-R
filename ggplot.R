...
sepal_plot1 <- ggplot(iris,aes(x = Sepal.Length, y = Sepal.Width, col = Species)) 
...
sepal_plot2 <- ggplot(iris,aes(x = Sepal.Length, y = Sepal.Width, col = Species, size = Petal.Length))  
...
jitter_pos <- position_jitter(width = 0.7)
ggplot(diamonds,aes(x = clarity,y = carat,col = price)) + geom_jitter(position = jitter_pos)
...
ggplot(diamonds,aes(x = carat,y = price, col = table)) + geom_point(alpha = 0.5) + facet_wrap(~cut)
